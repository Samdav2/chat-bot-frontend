...
import { User, MessageSquare, Send, StickyNote, X } from 'lucide-react';
...
export const CustomerDetails: React.FC<CustomerDetailsProps> = ({
  ...
  const content = (
    <div className="flex flex-col h-full space-y-6 overflow-y-auto p-5">
      {/* Header with optional close button */}
      <div className="flex items-center justify-between pb-2 border-b border-slate-800 xl:hidden">
  ...
  return (
    <>
      {/* Desktop Column View */}
      <div className="w-72 xl:w-80 hidden xl:flex flex-col h-full bg-slate-900 border-l border-slate-800 shrink-0">
        {content}
      </div>

      {/* Mobile/Tablet Slide-over Modal Overlay */}
      {isOpenMobile && (
        <div className="fixed inset-0 z-50 xl:hidden flex justify-end bg-black/70 backdrop-blur-sm animate-fadeIn">
          <div
            className="fixed inset-0"
            onClick={onCloseMobile}
          />
          <div className="relative w-80 max-w-full h-full bg-slate-900 border-l border-slate-800 shadow-2xl z-10 animate-slideLeft">
            {content}
          </div>
        </div>
      )}
